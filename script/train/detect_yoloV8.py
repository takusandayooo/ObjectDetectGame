...
def make_box_text_from_yoloV8(paths, full_path):
    model = YOLO("{}/yolov8x.pt".format(full_path))
    model = model.to("cuda")
    for path in tqdm(paths, desc="yoloV8"):
        im1 = Image.open(path)
        results = model.predict(source=im1, verbose=False)  # save plotted images
        items = results[0]
        for item in items:
            cls = int(item.boxes.cls)  # cls, (N, 1)
            label = item.names[int(cls)]
            score = item.boxes.conf.cpu().numpy()[0]  # confidence score, (N, 1)
            x1, y1, x2, y2 = item.boxes.xyxy.cpu().numpy()[
                0
...
if __name__ == "__main__":
    make_box_text_from_yoloV8(["test.jpg"], "../..")
